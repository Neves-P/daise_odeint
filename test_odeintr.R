...
eq_system <- 'dxdt[0] = lambdaA * x[1] + lambdaC * x[1] + mu * x[1] + lambdaC * n * x[0] + mu * n * x[0] - (mu + lambdaC) * n * x[0] - gam * x[0]; dxdt[1] = gam * x[0] + lambdaC * n * x[1] + mu * n * x[1] - (mu + lambdaC) * n * x[1] - lambdaA * x[1]; dxdt[2] = -(lambdaA + lambdaC + gam + mu) * x[2];'
